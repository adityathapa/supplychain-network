...
describe('Starting tests for supplychain-app', function() {
  ...
  it('website title should be supplychain-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('supplychain-app');
    })
  });
  ...
  it('navbar-brand should be supplychain-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('supplychain-app');
    });
  });
    ...
    it('commodity table should have 11 columns',() => {
      page.navigateTo('/commodity');
